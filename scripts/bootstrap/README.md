...
Автотесты библиотеки (без изменения hostname/firewall/дисков):

```bash
bash scripts/bootstrap/test-bootstrap.sh
```

Ожидается `PASS=… FAIL=0`. Полный прогон на РЕД ОС / Astra с реальным DATA-диском — только на целевых ВМ (см. архитектуру).
